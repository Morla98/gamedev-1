...
class NotMyJob implements AchievementLogicInterface
{
    /**
     * @inheritDoc
     */
...
(SELECT
 ...
 'not-my-job' as "achievement_id",
 u.email as "user_email",
 (CASE WHEN COUNT(m.id) >= 10 THEN 10 ELSE COUNT(m.id) END) * 10 AS "progress",
 NOW() as "last_updated"
FROM main.users u
LEFT JOIN (SELECT DISTINCT "id", "user", "timestamp" FROM metrics.template_collector WHERE action='assigned-away') m ON (u.email=m.user)
GROUP BY(u.email))
