...
import com.unihannover.gamedev.services.*;
...
public class AppStartupRunner implements ApplicationRunner {
    ...
    @Autowired
    HookHandlerService hookHandlerService; // only for simulating a hook
    ...
    /**
     * Small test function to simulate a hook on the general collector
     */
    private void simulateTest(){
        List<User> userlist = httpService.getUsers();
        String usermail = "JhonDoes@email.com";
        if(userlist.size() != 0){
            usermail = userlist.get(0).getEmail();
        }
        hookHandlerService.handle(usermail);
